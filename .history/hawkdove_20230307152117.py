...
#Modify these variables to get different results
#######
MIN_FOOD_PER_ROUND = 30
MAX_FOOD_PER_ROUND = 70

ENERGY_REQUIRED_FOR_REPRODUCTION = 60	
ENERGY_LOSS_FROM_FIGHTING = 50
ENERGY = 5
#######

"""
ECO equilibrium (two species cohexist and reproduce)
...
Hawks supremacy
lign 231 -> increase the matting ratio

Starving (All dying (not enough food))
...
def compete(agent, nemesis, food):
	...
	if agent.agent_type == TYPE_HAWK and nemesis.agent_type == TYPE_HAWK:
		if (getAgentCountByType(TYPE_HAWK)/(getAgentCountByType(TYPE_HAWK)+getAgentCountByType(TYPE_DOVE))) > 0.28:
